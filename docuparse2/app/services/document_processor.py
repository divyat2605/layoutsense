...
# Process-local image cache: doc_id -> List[np.ndarray]
_page_cache: Dict[str, List[np.ndarray]] = {}
    ...
    async def parse(self, session: AsyncSession, doc_id: str) -> ParseResponse:
        ...
        try:
            for page_num, page_image in enumerate(pages, start=1):
                logger.info("Processing page %d/%d of '%s'", page_num, doc.total_pages, doc.filename)

                # Stage 1-2-3: OCR
                raw_output = self._ocr.run(page_image, page_number=page_num)

                # Layout analysis (LightGBM classifier)
                page_result = self._layout.analyse(raw_output, page_number=page_num)

                # LayoutLMv3 re-scoring for low-confidence regions
                if self._layoutlmv3.is_available and page_result.regions:
                    rescored = self._layoutlmv3.score_regions(
                        page_result.regions, page_image, rescore_threshold=0.70,
                    )
